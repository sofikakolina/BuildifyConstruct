...
const PaymentDocuments = ({task}) => {
	const [file, setFile] = useState(null);
	const [documents, setDocuments] = useState(task.paymentDocuments);
	...
	const idCurrentProject = useAppSelector(state => state.idCurrentProject.value);
		useEffect(() => {
			setDocuments(task.paymentDocuments)
		},[])
	...
	console.log(cost)
	return (
		<div className="flex flex-col gap-4 bg-white shadow-2xl p-4 border-2 rounded-lg">
			<h2 className="font-bold text-lg">Платежные Документы</h2>
